...
.pop .pop-form{display:flex;gap:10px;flex-wrap:wrap}
.pop .pop-form input{flex:1 1 150px;border-radius:999px;border:1px solid var(--line-strong);background:rgba(244,238,227,.05);
  color:var(--ink);font-family:'Assistant';font-size:15px;padding:12px 18px;outline:none;direction:ltr;text-align:right}
/* A flex item whose overflow is not visible gets an automatic minimum size of
   zero, so the button shrank under its own label and clipped it: "שלחו לי
   פרטים" needed 135px and was given 93px on every phone. The primary action of
   the conversion popup was unreadable, and nothing in the markup showed it. */
.pop .pop-form .btn{flex:0 0 auto}
...
/* Below this the phone and the button no longer share a line comfortably, so
   the button takes its own and reads as a deliberate full-width action. */
@media (max-width:430px){.pop .pop-form .btn{flex:1 0 100%;justify-content:center}}
