...
.prose a{color:var(--gold2);font-weight:700;text-decoration:underline;
  text-decoration-color:rgba(217,164,91,.45);text-decoration-thickness:1.5px;text-underline-offset:3px;
  border-radius:5px;padding:1px 3px;margin:-1px -3px;
  transition:color .35s var(--ease),background .35s var(--ease),text-decoration-color .35s var(--ease)}
.prose a:hover{color:#f6d9a0;background:rgba(217,164,91,.1);text-decoration-color:var(--gold2)}
...
/* The section that follows the logo strip zeroes its own top padding, so the
   content lands directly on the strip's bottom border. The gap is added here,
   on the adjacent sibling, so only this pairing gets it — the strip elsewhere
   on the site keeps the rhythm it already has. */
.clients + .sec, .clients + .sec-tight{margin-top:clamp(30px,4.5vw,58px)}
...
<link rel="icon" type="image/png" sizes="96x96" href="${root}assets/icon-96.png">
<link rel="icon" type="image/png" sizes="48x48" href="${root}assets/icon-48.png">
